...
All incidents must be reported immediately upon detection through the following channels:

* **Discord:** OET employees can report incidents in the `#iso-security` Discord channel, tagging the ISM and permanent members of the response team. Note that as this channel is accessible to all OET employees, you should use direct messages (DMs) instead if the incident is not sensitive or confidential.
