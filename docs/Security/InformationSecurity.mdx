...
- For Windows PCs, encryption is only available on Windows Pro, not the Home edition. Please ensure you select this as an option at checkout if possible. If not, please get financial approval to upgrade the OS on setup, and then do so.
...
The recommended tool for securely wiping company devices is [ShredOS](https://github.com/PartialVolume/shredos.x86_64). ShredOS boots from external media (e.g., USB flash drive or an external hard drive) and includes `nwipe` for HDD overwrites, `hdparm` for SATA SSD secure erase, and `nvme-cli` for NVMe sanitize/format operations.

Use this procedure when offboarding, or returning/replacing a company device:

1. Preparation: Write the ShredOS image to bootable external media (USB flash drive or external hard drive) using [Rufus](https://rufus.ie/) (Windows) or `dd` (Linux).
...
5. Evidence:
    - For `nwipe`: capture the success screen. ShredOS automatically saves a PDF certificate and log file to the boot media.
