...
1. Activate your account
   ...
   - Store your account backup information securely (definitely not inside 1Password, and ideally not on a device on which you use the 1Password platform)
...
4. Install the apps
   ...
   - To make things a bit easier, in settings, you may extend the duration that 1Password's browser extension remains unlocked, before it automatically locks and requires you to log in again
   - Also, to make things easier, you can (at least in some browsers, like Chrome) create an entirely separate browser profile, specifically for OET. This way, you can have your own personal password manager in your personal browser profile, and simultaneously log into the OET browser profile and use the 1Password extension in there
   - 1Password for desktop allows you to conveniently access your credentials without internet access (offline mode)
...
4. Store your passwords
   ...
   - When saving new credentials for things that are OET-related, please always use your OET email address. If you have old, OET-related credentials that use other email addresses, please update them to use your OEt email
...
* **Browser extensions cannot communicate with the 1Password desktop app**
    ...
    * Special case Ubuntu: Since Ubuntu 21.04, Ubuntu automaticaly installs the `snap` package of `firefox`, even if you install it using `apt` from the Mozilla ppa. You need to [follow this linked guide](https://balintreczey.hu/blog/firefox-on-ubuntu-22-04-from-deb-not-from-snap/) to disable this automatism, else your `firefox` installation will be overwritten with the next update with the `snap` version.
