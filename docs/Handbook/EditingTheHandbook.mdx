...
- Maintainer tags should be present on every page. We use email as a unique identifier. For example:
```
import MaintainerTag from "@site/src/components/MaintainerTag";
import ControlledDocBanner from "@site/src/components/ControlledDocBanner";

<MaintainerTag maintainerEmails={["name.lastname@openenergytransition.org"]} />
```
