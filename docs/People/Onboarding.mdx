...
1. The device you're working on has full disk encryption enabled. If one is being purchased, please add this option. If that's not possible at checkout, please activate it once you receive the device following [this guide](https://docs.google.com/document/d/1UYJPbWbLbDMCZZfcHcWRxu5qIl1KAmzEUk2naNtEVeY/edit?tab=t.0). In either instance, if you're not sure how, you can ask our Software Engineering Team or People Team.
2. Your email signature is formatted correctly (you can use Quintin's as a template) 
3. Your Discord name makes sense to other OETers
4. You type a message in our #welcome channel in Discord, so others can say hi to you (feel free to use someone else's message as a foundation).
